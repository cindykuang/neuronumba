...
demo_folder = '4_diff_atlases_same_granularity'
parcellation_names = ['HarvardOxfordMaxProbThr0', 'Schaefer17Networks', 'MIST', 'CraddockSCorr2Level']
base_dir = f'/mnt/storage/neuronumba/demos/{demo_folder}'
resolutions = ['096', '100', '103', '108']
...
for i, res in enumerate(resolutions):
    ...
    target_path = opj(base_dir, f'{res}-{parcellation_names[i]}', f'Data_Raw_{res}-{parcellation_names[i]}')
    utils.check_directory_exists(target_path)
    
    # make sure data produced folder exists
    Data_Produced_path = opj(base_dir, f'{res}-{parcellation_names[i]}', f'Data_Produced_{res}-{parcellation_names[i]}')
    ...
    for sub in current_subs:
        ...
        if os.path.isdir(opj(EBrains_path, 'structural', f'{res}-{parcellation_names[i]}', '1StructuralConnectivity', index)):
            shutil.copyfile(opj(EBrains_path, 'structural', f'{res}-{parcellation_names[i]}', '1StructuralConnectivity', index, 'Counts.csv'),
                            opj(target_path, f'sub-{sub}', 'Counts.csv'))
            shutil.copyfile(opj(EBrains_path, 'structural', f'{res}-{parcellation_names[i]}', '1StructuralConnectivity', index, 'Lengths.csv'),
                            opj(target_path, f'sub-{sub}', 'Lengths.csv'))
            
        # get the pre-parcellated data and convert to .csv (weird spacing issues)?
        if os.path.isdir(opj(EBrains_path, 'parcellated_rsfMRI', f'{res}-{parcellation_names[i]}', index)):
            ...
            array = np.loadtxt(opj(EBrains_path, 'parcellated_rsfMRI', f'{res}-{parcellation_names[i]}', index, f'rfMRI_{session}_{run}_BOLD.tsv'))
