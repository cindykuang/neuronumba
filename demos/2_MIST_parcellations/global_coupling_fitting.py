...
def load_sc(fmri_path, scale):
    ...
    for path in os.listdir(fmri_path):
        ...
        if os.path.isdir(subject_path):
            ...
            if not os.path.isfile(fmri_file):
                raise FileNotFoundError(f"fMRI file <{fmri_file}> not found!")
            m = load_2d_matrix(fmri_file, delimiter=' ') #changed
            if sc is None:
                sc = m
            else:
                sc = sc + m
...
def run():
    ...
    if args.g is not None:
        ...
        # prepare common execution environment
        exec_env = {
            'verbose': True,
            'i': i,
            'model': copy.deepcopy(model),
        ...
        try:
            with ProcessPoolExecutor(max_workers=args.nproc) as pool: #changed
                futures = {
                    pool.submit(compute_g, exec_env, g): g
                    for g in gs
                }
                ...
                for future in as_completed(futures):
                    g = futures[future]
                    try:
                        result = future.result() #key output variable: results
                        results.append(result)
                        print(f"EXECUTOR --- FINISHED process for g={g}")
                    except Exception as exc:
                        print(f"EXECUTOR --- FAIL for g={g}: {exc}")
                        failed_gs.append(g)

        except KeyboardInterrupt: #changed
            print("KeyboardInterrupt caught in main process. Shutting down executor and cancelling tasks...") #changed
            pool.shutdown(wait=False, cancel_futures=True) #changed
            sys.exit(1) #changed

        if failed_gs:
            print(f"Failed to compute for global coupling values: {failed_gs}")
